...
public class VenteController implements Initializable {
    ...
    /**
     * The table field represents the table view that displays the list of all new sells "Vente"
     */
    ...
    /**
     * The Fieldref field is a combobox which items are all of the IDS of existing products in our database "Inventaire.fxml". The IDS are loaded by code in this class
     */
    @FXML
    ...
    /**
     * The field button is the button "Confirmer" that triggers the sell operation 
     */
    @FXML
    ...
    /**
     * The field combofclient field is a combobox containing a list of items which are all of the clients of the store
     * Contains the names of the clients stored in "Clients.xlsx"
     */
    @FXML
    ...
    private AnchorPane grid;
     ...
     /**
     * Collection containing the list of products in the database "Inventaire.xlsx" for further manipulation
     * We use it to construct the table view and alter the quantity of a product after a sell opperaton
     */
    private ArrayList<ArrayList<String>> sourceData =new ArrayList();
    /**
     * Collection storing the list of clients from "Clients.xlsx"
     * We use it to populate the combobox containing the list of all clients
     */
    ...
    private ExcelWorksheet myworksheet;
     /**
     * Workbook of the excel file "Inventaire.xlsx" which is our main database of the stock
     */
    private ExcelFile myworkbook;
    private ExcelWorksheet myjournalsheet;
    /**
     * Workbook of the excel file Journal.xlsx storing the movements in the stock after completing the buying opperation in the form of a bill
     */
    private ExcelFile myjournalbook;
    private ExcelWorksheet myclientsheet;
      /**
     * Workbook of the excel file storing the portfolio of clients "Clients.xlsx"
     */
   ...
   static boolean idexist=true;
     ...
     /**
      * This method is automatically called after the fxml file has been loaded<br><br>
     * Initilizes the controller class, sets up the table view and all of the other controls in the fxml file <br>
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {  
			...
			dialogContent.addActions(
					Map.entry(new MFXButton("Confirmer"), event -> {dialog.close();
					}),
			...
			dialogContent1.addActions(
					Map.entry(new MFXButton("Confirmer"), event -> {dialog1.close();
					}),
					Map.entry(new MFXButton("Annuler"), event -> {
                                            myevent.consume();
			...
			dialogContent2.addActions(
					Map.entry(new MFXButton("Confirmer"), event -> {dialog2.close();
					}),
					Map.entry(new MFXButton("Annuler"), event -> {
			...
			dialogContent3.addActions(
					Map.entry(new MFXButton("Confirmer"), event -> {dialog3.close();
					}),
					Map.entry(new MFXButton("Annuler"), event -> {
                                            dialog3.close();
   ...
   /**
     * Event handler for the button "Confirmer" which triggers the sell operation
     * It alters the stock and creates a new row in the tableview
    ...
    if(idexist)
    {
     ...
     /**
     * Searches for the row in the databases with a specific id. Ie the product with the specified ID<br><br>
     * Takes dataS : Our data source containing all of the products in the database "Inventaire.xlsx" and the ID of the product we're looking for<br>
     * returns an ArrayList of strings  which is a row in the databse containing the product with the specified ID<br>
     */
    ...
    /**
     * Searches for the row in the databases "Clients.xlxsx" with the specific name 
     * Takes : Our data source containing all of the providers "client" and the name of the cleint we're looking for
     * Retruns an ArrayList of strings which is a row in the database containing clients with the specified name
     */
